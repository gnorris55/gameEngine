...
public:
    ...
    void set_depth_map(std::vector<Entity*> entities, Shader *depth_shader, bool debug = false) {
       ...
       // glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        ...
        // causes all depth values to be max
        ...
        depth_shader->setVec3("lightPos", position);
...
public:
    ...
    LightManager(Shader* shader) : shader(shader) {
        load_model();
        this->depth_shader = new Shader(RESOURCES_PATH"shaders/shadowMapRendering/omniDirectionalLight/depthShader.vs", RESOURCES_PATH"shaders/shadowMapRendering/omniDirectionalLight/depthShader.fs", RESOURCES_PATH"shaders/shadowMapRendering/omniDirectionalLight/depthShader.gs");
    ...
    void set_depth_maps(std::vector<Entity *> entities) {
       
        for (Light* light : lights)
            light->set_depth_map(entities, this->depth_shader, false);
