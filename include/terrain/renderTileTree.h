...
public:
	...
	~RenderTileTree() {
		...
		glDeleteShader(clipping_shader->ID);
		glDeleteShader(grass_shader->ID);
	}


	void draw(float time, glm::mat4 proj, glm::mat4 view, glm::vec3 view_pos, glm::vec3 view_normal, Frustum* frustum) {
		get_clipping_tex(proj * view, view_pos);
		render_grass_tiles(time, proj * view, view_pos, view_normal, frustum);
	}
...
private:
	...
	void load_textures() {
		...
		grass_heights_tex = loader.load_texture(RESOURCES_PATH"textures/grass/clumping2.jpg");
		grass_color_tex = loader.load_texture(RESOURCES_PATH"textures/grass/grassColor.jpg");
		wind_tex = loader.load_texture(RESOURCES_PATH"textures/grass/wind.jpg");
	...
	//drawing
	void render_grass_tiles(float time, glm::mat4 proj_view, glm::vec3 view_pos, glm::vec3 view_normal, Frustum* frustum) {

		grass_shader->use();
		grass_shader->setVec3("viewPos", view_pos);
		grass_shader->setVec3("viewNormal", glm::normalize(view_normal));
